...
func (a *Api) Register(container *restful.Container) {
	...
	//#-CK_FEATURE_MAGIC-#  tanck eagletrackSystemReady  20170206 "A example for eagle track system ready"
	/*#+ add by tanck, and feature track endpoint +#*/
	ws = new(restful.WebService)
	ws.Path("/eagletrackfeatures").
		Doc("custom features by heapster").
		Produces(restful.MIME_JSON)
	ws.Route(ws.GET("").
		To(getEagletrackFeatures).
		Param(ws.QueryParameter("detail", "whether to show feature details").DataType("string")).
		Doc("get custom features").
		Operation("getEagletrackFeatures").
		Writes([]interface{}{}))
	container.Add(ws)
	/*#- add by tanck, and feature track endpoint -#*/
...
/*#+ add by tanck, and feature track endpoint +#*/
func getEagletrackFeatures(request *restful.Request, response *restful.Response) {
	detail := request.QueryParameter("detail")
	if len(detail) > 0 && detail == "true" {
		response.WriteEntity(eagleeye.CkFeatureList)
		return
	}

	var result []string
	for _, f := range eagleeye.CkFeatureList {
